...
# Global cancellation tracking - SIMPLE and EFFECTIVE
cancellation_flags = {}  # user_id -> True if cancelled
active_downloads = {}  # user_id -> message_id for active downloads
...
def check_cancellation(user_id, message_id=None):
    """Check if download should be cancelled"""
    if user_id in cancellation_flags and cancellation_flags[user_id]:
        # Clear the flag after checking
        cancellation_flags.pop(user_id, None)
        # Also clear from active downloads
...
def progress(current, total, message, type):
    ...
    try:
        ...
        # Check for cancellation every time progress is called
        check_cancellation(user_id, message.id)
...
async def send_cancel(client: Client, message: Message):
    ...
    # Set cancellation flag - SIMPLE AND EFFECTIVE
    cancellation_flags[user_id] = True
    
    # Also stop any ongoing downloads immediately
    ...
    await message.reply_text("✅ **Cancellation request sent!**\n<i>Active download will be stopped immediately.</i>", parse_mode=enums.ParseMode.HTML)
    
    # Clear the flag after 30 seconds to allow new downloads
    ...
    if "https://t.me/" in message.text:
        ...
        for msgid in range(fromID, toID + 1):
            ...
            try:
                ...
                active_downloads[user_id] = message.id
