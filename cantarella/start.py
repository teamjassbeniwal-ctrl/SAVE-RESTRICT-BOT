...
class batch_temp(object):
    IS_BATCH = {}
    CANCELLED_USERS = {}  # Track users who cancelled with timestamp
...
def progress(current, total, message, type):
    ...
    # Check if user cancelled recently (within last 10 seconds)
    if user_id in batch_temp.CANCELLED_USERS:
        cancelled_time = batch_temp.CANCELLED_USERS[user_id]
        if time.time() - cancelled_time < 30:  # 30 seconds timeout
...
async def send_cancel(client: Client, message: Message):
    ...
    try:
        # Find and delete all status files for this user
    ...
    await message.reply_text("🚫 **Cancelling all downloads...**\n<i>Please wait a moment...</i>", parse_mode=enums.ParseMode.HTML)
    
    # Wait a bit and send final confirmation
    await asyncio.sleep(1)
    await message.reply_text("✅ **All downloads cancelled successfully!**\n<i>You can now start new downloads.</i>", parse_mode=enums.ParseMode.HTML)
    
    # Clean up flags after delay
        ...
        for msgid in range(fromID, toID + 1):
            ...
            if is_public_link:
                ...
                try:
                    ...
                    success_count += 1
                    await asyncio.sleep(0.5)  # Reduced delay for premium
                    continue
                except Exception as e:
            ...
            try:
                # Configure client based on premium status for faster speeds
                if is_premium:
                    # Premium gets more concurrent transmissions
                    acc = Client(
                        "saverestricted",
                        ...
                        api_id=API_ID,
                        in_memory=True,
                        max_concurrent_transmissions=20,  # More for premium
                        sleep_threshold=0,  # No sleep threshold for faster speed
...
async def handle_restricted_content(client: Client, acc, message: Message, chat_target, msgid, success_count):
    ...
    try:
        ...
        # Download file with different chunk sizes based on premium status
        chunk_size = 1024 * 1024 * 10 if is_premium else 1024 * 512  # 10MB chunks for premium, 512KB for free
        ...
        await asyncio.sleep(0.1)  # Small delay to start
    ...
    try:
        ...
        # Send file based on type with different chunk sizes for premium
        if is_premium:
            # Premium users get larger chunk sizes for faster upload
            ...
            # Free users get standard chunk sizes
            if msg_type == "Document":
                await client.send_document(
                    message.chat.id, 
                    file, 
