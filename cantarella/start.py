...
# Simple global cancellation tracking
active_downloads = {}  # user_id -> {"cancelled": bool, "last_check": time}
download_lock = threading.Lock()

def check_cancellation(user_id):
    """Check if user has cancelled their download"""
    with download_lock:
        if user_id in active_downloads:
            if active_downloads[user_id].get("cancelled", False):
                # Clear after checking
                active_downloads.pop(user_id, None)
                return True
            # Update last check time
            active_downloads[user_id]["last_check"] = time.time()
        return False

def mark_cancelled(user_id):
    """Mark user's download as cancelled"""
    with download_lock:
        if user_id in active_downloads:
            active_downloads[user_id]["cancelled"] = True
        else:
            active_downloads[user_id] = {"cancelled": True, "last_check": time.time()}

def register_download(user_id):
    """Register a new download for a user"""
    with download_lock:
        active_downloads[user_id] = {"cancelled": False, "last_check": time.time()}

def unregister_download(user_id):
...
async def send_cancel(client: Client, message: Message):
    ...
    await message.reply_text("✅ **Cancellation request sent!**\n<i>The download will be stopped as soon as possible.</i>", parse_mode=enums.ParseMode.HTML)
        ...
        try:
            for msgid in range(fromID, toID + 1):
                ...
                if user_data is None:
                    ...
                    unregister_download(user_id)
                    return
                
                try:
                    # Create session with optimized settings for speed
                    session_name = f"saverestricted_{user_id}_{int(time.time())}"
                    if is_premium:
                        # Premium: Fast settings
                        acc = Client(
                            session_name,
                            ...
                            api_id=API_ID,
                            in_memory=True,
                            max_concurrent_transmissions=20,  # More concurrent for premium
                            sleep_threshold=0,  # No sleep for premium
                            workers=4  # More workers for premium
                        )
                    else:
                        # Free: Standard settings
                        acc = Client(
                            session_name,
                            session_string=user_data,
                            api_hash=API_HASH,
                    ...
                    if is_private_link:
                        ...
                        success = await handle_restricted_content(client, acc, message, chatid, msgid, success_count, user_id)
                        if success:
                            success_count += 1
                    elif is_batch:
                        username = datas[4]
                        success = await handle_restricted_content(client, acc, message, username, msgid, success_count, user_id)
                        if success:
                            success_count += 1
                    else:
                        username = datas[3]
...
async def handle_restricted_content(client: Client, acc, message: Message, chat_target, msgid, success_count, user_id):
    is_premium = await db.check_premium(user_id)
